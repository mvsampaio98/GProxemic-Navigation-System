...
MAX_X1=MAP_CORDINATES.Info.Height; %%70 %50
MAX_X=double(MAX_X1);
MAX_Y1=MAP_CORDINATES.Info.Width; %%70  %40
MAX_Y=double(MAX_Y1);
AUXILIAR_MAPT = AUXILIAR_MAP';
...
xval= 25 * (abs(MAP_CORDINATES.Info.Origin.Position.Y));
yval= 25 * (abs(MAP_CORDINATES.Info.Origin.Position.X));
%xStart = 425;
%yStart = 125;
xStart = xval;
yStart = yval;
...
while but == 1
    ...
    gau=PruebaGausian(xval,yval,MAX_X ,MAX_Y,division,rotacion);
    contour3(x+0.9,y+0.8,gau,4)%0.8x
...
for i=1:size(Ruta_final, 1)
    while cmd_vel_msg.Goal.TargetPose.Pose.Position.X>((Ruta_final(i,1)/25)+1) | cmd_vel_msg.Goal.TargetPose.Pose.Position.X<((Ruta_final(i,1)/25)-1) | cmd_vel_msg.Goal.TargetPose.Pose.Position.Y>((Ruta_final(i,2)/25)+1) | cmd_vel_msg.Goal.TargetPose.Pose.Position.Y<((Ruta_final(i,2)/25)-1)
        cmd_vel_msg.Header.Seq = i-1;
        cmd_vel_msg.Goal.TargetPose.Header.Seq = i-1;
        cmd_vel_msg.Goal.TargetPose.Header.FrameId = 'odom';
        cmd_vel_msg.Goal.TargetPose.Pose.Position.X = Ruta_final(i,1)/25;
        cmd_vel_msg.Goal.TargetPose.Pose.Position.Y = Ruta_final(i,2)/25;
